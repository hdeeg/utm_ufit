...
;  deminiter the '&' chracters 8as in a tex table. It ignores lines starting 
     ...
     if isa(datarr) then begin ;generate datarr output string array
        for k=0L,ndat-1 do begin
           ...
           if ntok gt noutcol then if not(keyword_set(maxcol)) then $
              if ~keyword_set(nowarn) then tprint,'WARN: ',ntok,' tokens in data-line ',k,' versus ',noutcol,' tokens in data-array';more token in a data-line
           
           if ntok ge noutcol then datarr[k,*]=tok[0:noutcol-1] else begin 
               if ~keyword_set(nowarn) then print,'WARN:',ntok,' tokens in data-line ',k,' versus ',noutcol,' tokens in data-array' ;less token in a data-line
