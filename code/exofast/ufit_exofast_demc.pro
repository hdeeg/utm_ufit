...
;   points of each chain.  (for UFIT: modified to 2*scale*random)
;
...
;
; CALLING SEQUENCE:
;   ufit_exofast_demc, bestpars, 'mychi2', pars [,CHI2=, TOFIT=,$
;                 SCALE=, SEED=,RANDOMFUNC=,NTHIN=, MAXSTEPS=,/DONTSTOP,$
;                 NCHAINS=, ANGULAR=,BURNNDX=,DCMULT=,INITMULT=,/REMOVEBURN]
;
; INPUTS:
...
;                parameter array.
;   STOPSCALE  - If set, stops after determining the scale
;   DCMULT     - In get_mcmcscale, changes stopping criterium from 
;                delta chi2=1 to delta chi^2= DCMULT. The obtained 
;                scales are then divided by DCMULT
;                to correspond again to a  delta chi^2=1.
;   INITMULT   - by default, initial parameters are scattered within 2 times 
;                of their scale-sizes. (In original exofast_demc this was 5x).
;                INITMULT may change this from 2x to other multipliers  
; OUTPUTS:
...
;   2015/09/16 - H.Deeg: version for ufit. Added stopscale keyw. 
;                          Modified initial chain parameter offset from 5x 
;              to 2x. 
;          Added some printing stuff (acc, rej:...)
;   2019/02/21 - HD: added condition that program is exited after
;       ufit_exofast_getmcmcscale, if stopcale =1. Added common block for dflag
;   2019/03/09  HD: DCMULT keyword for propagation to ufit_exofast_getmcmcscale
;   2019/03/17  HD: modified so that scales obtained from ufit_exofast_getmcmcscale are divided by DCMULT.
;   2019/04/17 fixed bug that produced error if scales were previously known
;   2019/05/05 HD: added initmult keyword
;   27/07/20    HD: added _extra kw in order to get further params to getmcmcscale.pro
;   25/08/20   HD: minor fix to supress all printing for dflag =0

;
pro ufit_exofast_demc, bestpars,chi2func,pars,chi2=chi2, tofit=tofit,$
                       ...
                       stopscale=stopscale, dcmult=dcmult, $
                       initmult=initmult, _extra=ex
...
;; get the MCMC step scale for each parameter
  if dflag ge 2 then debug= 1 else debug =0
  if n_elements(scale) eq 0 then begin
     scale = ufit_exofast_getmcmcscale(bestpars,chi2func,tofit=tofit,angular=angular,dcmult=dcmult,debug=debug,_extra=ex)
     scale=scale/dcmult         ;scale the scales back to a delta-chisq=1
  endif
  if scale[0] eq -1 then message, 'No scale found'
  if dflag ge 1 then print,'Scales  have been determined'
  if dflag ge 1 then print,'Scale =', scale
  if keyword_set(stopscale) then begin
     goto, jumpout_getmcmcscale_only
  endif
;stop

  stopit = 0
;; initialize each chain
  if ~keyword_set(initmult) then initmult=2.
...
;; HJD: modified to start within initmult x scalesizes from best value

        pars[0:nfit-1,0,j] = bestpars[tofit] + $
                             initmult*scale*call_function(randomfunc,seed,nfit,/normal)
  ...
  print,'Chains have been initialized'
  nextrecalc = 100L
  npass = 1L 
  nstop = 0L
  naccept = 1d0
...
;print,'OP:',oldpars[tofit]           
;print,'NP:',newpars[tofit]           
           ...
           testval= call_function(randomfunc,seed)
           if testval lt C then begin
              if dflag ge 2 then print,'acc: testval:',testval,' C:',C,' newchi2:', newchi2,' oldchi2',oldchi2
              naccept++
              olddet[j] = det
              oldpars = newpars
              oldchi2 = newchi2
           endif else begin
              if dflag ge 2 then print, 'rej: testval:',testval,' C:',C,' newchi2:', newchi2,' oldchi2',oldchi2
           endelse
...
;; if hit maxsteps and never converged
  if npass ne 6 then begin
     ...
     if bad[0] ne -1 then begin
        ...
        for tmp=0,n_elements(bad)-1 do print, tofit[bad[tmp]], gelmanrubin[bad[tmp]],tz[bad[tmp]]
...
;; calculate the runtime and units
  ...
  jumpout_getmcmcscale_only: dummy=[] 
end
